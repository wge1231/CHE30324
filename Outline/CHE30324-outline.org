...
2. Maxwell-Boltzmann distribution of molecular speeds (Figure 3)
   ...
   2. $P_{MB}(v) dv = P_{1D}(v_x) P_{1D}(v_y) P_{1D}(v_z) * \text{degeneracy}(v) dv$ (Table 4)
   3. mean speeds $\langle v \rangle = \int_{-\infty}^{\infty} v P_{MB}(v)dv \propto \sqrt{T}$
   4. mean kinetic energy $\langle U \rangle = \frac{1}{2} m \langle v^2 \rangle =\frac{3}{2} RT$
   5. heat capacity $C_v= dU/dT = \frac{3}{2} R$
3. Flux and pressure
...
5. See Table 4 for important equations
