...
3. Characteristic of light, among other thing
4. Expected energy of a classical wave, $\langle \epsilon \rangle _\nu = k_B T$ for all $\nu$
...
1. Blackbody/Hohlraum spectrum (like the sun)
   ...
   2. Wien's displacement law, $\lambda_\mathrrm{text}T = \mathrm{constant}$
...
3. Planck model, 1900
   ...
   2. Expected energy of a quantized wave, $\langle \epsilon \rangle_\nu = h\nu/\left (
          e^{h\nu/k_BT}-1 \right )$
...
3. Einstein model, 1905 (Nobel prize)
...
*** Compton effect
1. light scattering off electrons changes $\lambda$
2. Demonstrates that photons carry momentum, $p=h/\lambda$
