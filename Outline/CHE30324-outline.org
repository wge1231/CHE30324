...
- Energy of a closed system is conserved (first law!)
- Degeneracy of total system is always $\geq$ degeneracy of the starting parts!
- Boltzmann's tombstone, $S = k_B \ln \Omega$
- Clausius: entropy of the universe seeks a maximum!  Second Law...
...
3. Probability for system to be in a microstate with energy $U_j$ given by Boltzmann distribution!
        ...
        \end{displaymath}
4. Partition function "normalizes" distribution, $Q(T) = \sum_j e^{-U_j \beta}$
...
2. "Standard state" corresponds to some standard choice for $N/V$, $c^\circ$
3. For ideal gas, related to pressure by $P^\circ = c^\circ k_B T$
*** Chemical reaction $A \rightarrow B$
1. Reaction entropy $\Delta S^\circ (T) =  S^\circ_\mathrm{B}(T)-S^\circ_\mathrm{A}(T)$
2. Reaction energy $\Delta U^\circ (T) = U^\circ_\mathrm{B}(T)-U^\circ_\mathrm{A}(T)+\Delta E(0)$
3. Equilibrium condition---equate chemical potentials, $\mu_A(N,V,T) = \mu_B(N,V,T)$
4. Equilibrium constant---evaluate from partition functions directly
...
2. Arrhenius expression, $k=A e^{-E_a/k_BT}$
*** Reaction mechanisms
1. Elementary steps and molecularity
2. Collision theory---overpredicts rates
