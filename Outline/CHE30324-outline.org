...
2. Principle quantum number $n=1,2,...$
  1. $K$, $L$, $M$, $N$, ... shells
  2. $n-1$ radial nodes
...
7. Electronic selection rules
   1. $\Delta l=\pm 1$, $\Delta m_s =0$, $\Delta m_l = 0,\pm 1$
8. Wavefunctions = "orbitals"
9. Radial probability function $P_{nl}(r)=r^2 R_{nl}^2(r)$
