...
*** Chemical reaction
1. Consider \ce{A -> B}, eg vinyl alcohol to acetaldehyde (\ce{(HO)HC=CH2 -> CH3CHO})
2. Standard reaction entropy $\Delta S^\circ (T) =  S^\circ_\mathrm{B}(T)-S^\circ_\mathrm{A}(T)$
3. Reaction energy must capture difference in \SI{0}{K} electronic energy
   \begin{displaymath}
 \Delta U^\circ (T) = U^\circ_\mathrm{B}(T)-U^\circ_\mathrm{A}(T)+\Delta E(0)
    \end{displaymath}
4. Standard reaction free energy
*** Chemical equilibrium 
1. Consider \ce{A <=> B}, eg vinyl alcohol to acetaldehyde
2. At chemical equilbrium, total free energy minimized with respect to reaction advancement
3. Convolution of energy and entropy effects
4. Equilibrium condition---equate chemical potentials
   ...
   \frac{N_B}{N_A} =  \frac{N_B/V}{N_A/V} & = &\frac{q_B(T,V)/V}{q_A(T,V)/V} e^{-\Delta E(0)/kT}
     \end{eqnarray*}
5. Equilibrium constant---specify standard state to eliminate volume dependence
    \begin{eqnarray*}
...
def qrot(T,Trot):
    return np.sqrt((np.pi*(T/Trot[0])*(T/Trot[1])*(T/Trot[2])))
...
TrotVA = [3.06869, 0.49808, 0.42852]   # Kelvin
TvibVA = [342.12,   692.39,  1032.15,  1198.36,  1386.62, 1411.99,  1670.11,  1880.98,  1967.27,  2101.01, 2532.91,  4541.20,  4591.62,  4716.63,  5454.14]  # Kelvin
EVA = -153.802227784  # au

TrotAc = [ 2.72923, 0.48351, 0.43371]
TvibAc = [222.94,   729.92,  1121.87,  1285.63,  1636.17, 1646.79,  2018.97,  2080.76,
  2143.85,  2157.44,  2651.18,  4169.12,  4379.22,  4459.71,  4553.14]
EAc = -153.830119460 # au

T = np.linspace(200,1000,500)

qrotVA = qrot(T,TrotVA)
qrotAc = qrot(T,TrotAc)
qvibVA = qvib(T,TvibVA)
qvibAc = qvib(T,TvibAc)
...
UrotVA = Urot(T,TrotVA)
UrotAc = Urot(T,TrotAc)
UvibVA = Uvib(T,TvibVA)
UvibAc = Uvib(T,TvibAc)

SrotVA = S(ArotVA,UrotVA,T)
SrotAc = S(ArotAc,UrotAc,T)
SvibVA = S(AvibVA,UvibVA,T)
SvibAc = S(AvibAc,UvibAc,T)

deltaSrot = SrotAc-SrotVA
deltaSvib = SvibAc-SvibVA
deltaS = deltaSrot+deltaSvib

plt.figure()
plt.plot(T,deltaSrot*eVtokJ,label="rot")
plt.plot(T,deltaSvib*eVtokJ,label="vib")
plt.plot(T,deltaS*eVtokJ,label="total")
...
deltaZPE = ZPE(TvibAc)-ZPE(TvibVA)

plt.plot(T,deltaUrot*eVtokJ,label="rot")
plt.plot(T,deltaUvib*eVtokJ,label="vib")
plt.plot([200,1000],[deltaZPE*eVtokJ,deltaZPE*eVtokJ],label="ZPE")
plt.xlabel('Temperature (K)')
